...
for(i in 1:nrep){
  ...
  #---------
  write.table(x = cbind(group, Y), file = paste0("~/work/nested/low_dim_low_sep/data/rep_", i, ".csv"), row.names = F, col.names = F, sep = ",")
  write.table(x = cbind(part_true), file = paste0("~/work/nested/low_dim_low_sep/true_part/rep_", i, ".csv"), sep = ",", row.names = F, col.names = F)
}
