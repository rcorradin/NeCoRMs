...
double lnormpdf(vec x,
                vec mean,
                mat Sigma){
  int d = x.n_elem; 
  double out, rootisum, constants;
  mat rooti = inv(trimatu(chol(Sigma))).t();
  
  rootisum = sum(log(rooti.diag()));
  constants = -d/2.0 * log2pi;
  vec z = rooti * (x - mean);
...
void update_eta(vec u, 
  ...
  eta_location.resize(eta_weights.n_elem, m0.n_elem);
  eta_scale.resize(m0.n_elem, m0.n_elem, eta_weights.n_elem);
  
  for(uword j = 0; j < eta_weights.n_elem; j++){
    eta_scale.slice(j) = inv(wishrnd(inv(S0), n0));
    eta_location.row(j) = mvnrnd(m0, eta_scale.slice(j)/k0).t();
  }
}
      ...
      for(uword n = 0; n < Y.n_rows; n++){
        ...
        if(group(n) == l){
          for(uword j = 0; j < sigmal.n_elem; j++){
            ...
            temp_val += exp(log(sigmal(j)) + log(Tlj(i, j)) + 
              lnormpdf(Y.row(n).t(), active_location.row(j).t(), active_scale.slice(j)));
          }
          for(uword j = 0; j < eta_weights.n_elem; j++){
            accu_mass += eta_weights(j) * mij(i, j);
            temp_val += exp(log(eta_weights(j)) + log(mij(i, j)) + 
              lnormpdf(Y.row(n).t(), eta_location.row(j).t(), eta_scale.slice(j)));
          }
          temp_prob(i) += log(temp_val + pow(10, -255)) - log(accu_mass);
        }
      }
    }
    // Rcpp::Rcout << temp_prob.t() << "\n";
...
void update_cluster_nested(mat Y, 
                           vec &clust, 
                           vec group, 
                           vec nested_c,
                           vec eta_weights, 
  ...
  for(uword i = 0; i < clust.n_elem; i++){
    ...
    // Rcpp::Rcout << "inside_1 " << i << "\n";
    for(uword j = 0; j < sigmal.n_elem; j++){
      // Rcpp::Rcout << sigmal(j) << "\n" 
      //             << Tlj << "\n" 
      //             << Y.row(i) << "\n" 
      //             << nested_c << "\n";
      prob(j) = log(sigmal(j)) + log(Tlj(nested_c(group(i)), j)) + 
        lnormpdf(Y.row(i).t(), active_location.row(j).t(), active_scale.slice(j));
    }
    // Rcpp::Rcout << "inside_2 " << i << "\n";
    for(uword j = 0; j < eta_weights.n_elem; j++){
      prob(sigmal.n_elem + j) = log(eta_weights(j)) + log(mij(nested_c(group(i)), j)) + 
        lnormpdf(Y.row(i).t(), eta_location.row(j).t(), eta_scale.slice(j));
    }
    // Rcpp::Rcout << "inside_3 " << i << "\n";
    clust(i) = rint_not_unif_log(prob);
  }
}
...
void accelerate_location_scale(mat data,
                               vec clust,
                               mat &mu,
                               cube &s2,
                               vec m0,
  ...
  for(arma::uword j = 0; j < mu.n_rows; j++){
    ...
    kn = k0 + nj;
    mn = ((m0 * k0) + (sum(tdata, 0)).t())/kn;
    cdata = tdata - repmat(mean(tdata, 0), nj, 1);
    nn = n0 + nj;
    Sn = S0 + cdata.t() * cdata + ((k0*nj)/kn) *
      ((mean(tdata, 0)).t() - m0) * ((mean(tdata, 0)).t() - m0).t();
    
    s2.slice(j) = inv(wishrnd(inv(Sn), nn));
    mu.row(j) = trans(mvnrnd(mn, s2.slice(j)/kn));
  }
}
...
mat eval_dens_comp_nested(int d,
  ...
  for(uword r = 0; r < d; r++){
    ...
    for(uword j = 0; j < sigmal.n_elem; j++){
      ...
      for(uword i = 0; i < grid.n_rows; i++){
        res(r,i) += sigmal(j) * Tlj(nested_c(r), j) * 
          exp(lnormpdf(grid.row(i).t(), active_location.row(j).t(), active_scale.slice(j)));
      }
    }
    ...
    for(uword j = 0; j < eta_weights.n_elem; j++){
      ...
      for(uword i = 0; i < grid.n_rows; i++){
        res(r,i) += eta_weights(j) * mij(nested_c(r), j) * 
          exp(lnormpdf(grid.row(i).t(), eta_location.row(j).t(), eta_scale.slice(j)));
      }
    }
...
Rcpp::List main_nested(mat Y, 
  ...
  mat active_location(1, Y.n_cols);
  cube active_scale(Y.n_cols, Y.n_cols, 1);
  ...
  for(uword iter = 0; iter < niter; iter ++){
    ...
    // Rcpp::Rcout << "\nQUI3...\n" ;
    update_eta(u, sigma, phi, m0, k0, S0, n0,
