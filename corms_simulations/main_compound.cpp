...
double integrate_levy_inversion(double temp_S,
  ...
  // double a = atan(J) * 2.0 / M_PI;
  // double b = 1.0;
  // double output = (1.0 / 8) * (integrand_levy_inversion(a, u, J, sigma, phi) +
  //   3 * integrand_levy_inversion(2 * a + b, u, J, sigma, phi) +
  //   3 * integrand_levy_inversion(a + 2 * b, u, J, sigma, phi) +
  //   integrand_levy_inversion(b, u, J, sigma, phi));
  // output *= (1.0 - atan(J) * 2.0 / M_PI);
...
Rcpp::List main_compound(vec Y, 
  ...
  for(uword iter = 0; iter < niter; iter ++){
    ...
    // 
    // update_phi_MH(sigma, phi, MH_var, freqs, active_location, u, sigmal, M);
    // Rcpp::Rcout << "\n" << sigma;
    // save the results
