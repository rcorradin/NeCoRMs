...
Rcpp::sourceCpp('main_compound.cpp')
...
writeLines(c(""), "output_50_high.txt")
out_list_50_high <- list()
...
for(i in 1:nrep){
  ...
  cat(paste("completed: ", rep / 100, "%\n"), file = "output_50_high.txt", append = TRUE)
  out_list_50_high[[i]] <- res
}
save.image(file = "simu_compound_50_high.Rdata")
